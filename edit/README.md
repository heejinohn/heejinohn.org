...
| `drafts/` | Private drafts (not in git) | Nowhere public; `make preview-drafts` only |
...
- **New blog post:** `make draft title=my-title` starts a private draft in
  `drafts/`; preview it with `make preview-drafts`. To publish, move it to
  `blog/`, set `draft: false`, commit and push.
